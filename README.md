...
`v0.3.0` 是 Codex-only 公开 alpha：新增 Source Turn 与 Turn-first Task 重建，让长
Session 中的多个真实目标、工具验证和跨 Session 关联保留可回查来源。它仍不包含 Claude
Code、多工具、周/月报、常驻服务、动态 Dashboard 或云端上传。

## 安装

支持 macOS Apple Silicon 和 Intel。安装器下载固定 `v0.3.0` asset，验证 SHA-256，安装到 `~/.local/bin/aci`，并配置当前用户的 19:00 LaunchAgent；全程不使用 `sudo`。
...
```sh
curl -fsSL https://github.com/PANGKAIFENG/ai-collaboration-insights/releases/download/v0.3.0/uninstall.sh | sh
```

显式删除全部应用派生数据：

```sh
curl -fsSL https://github.com/PANGKAIFENG/ai-collaboration-insights/releases/download/v0.3.0/uninstall.sh | sh -s -- --purge-data
```
