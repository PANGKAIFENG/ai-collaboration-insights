...
- risk labels、confidence 与 test tags
- 每项可评分证据的 availability

每个 prediction record 必须包含相同 `caseId` 的候选 task boundary、project、task name、deliverable、verification status、complexity、五维分数、availability、evidence gates、risk labels 和 confidence，并可选包含 `usageContext`。prediction 不得复制 gold、标注解释或任何真实会话内容。
...
Task 2 必须按以下最小 shape 校验 record：

- `evidenceWindow` 为 `{day, start, end}`；`day` 使用 `YYYY-MM-DD`，`start`/`end` 使用 RFC3339，且 `start < end`。
- gold 与 prediction 的 `taskBoundary` 同为 `{segmentIds}`；`segmentIds` 是非空、唯一、非空 string array。
- `dimensionScores` 必须精确包含 rubric 五个维度 key，每项是 `0..100` 数值或 `null`。
- gold 与 prediction 都必须包含 `availability`，并精确使用五个维度 key。某维度 `availability=unavailable` 当且仅当对应 score 为 `null`；`present` 或 `observed_absent` 时 score 必须是数值。
- `evidenceGates` 精确包含 `iteration`、`verification`、`asset`，值使用三态 availability；只有 `present` 计入 evidence count。
- `verificationStatus` 仅允许 `verified|unverified|unavailable`。`project`、`deliverable` 和 prediction `taskName` 均为非空 synthetic string；gold `acceptedTaskNames` 为非空、唯一 string array；`confidence` 为 `0..1` 数值。
- `riskLabels` 为唯一 string array。gold 另含 `testTags` string array；带 `usage_only_risk_negative` tag 的 case，其 prediction `riskLabels` 必须为空。
- 可选 `usageContext` 只能包含 rubric `prohibitedScoringInputs` 中的 key，且不得影响 complexity 或任何 score。
...
任一评分维度为 `unavailable` 时，对应 dimension score 必须为 `null`，任务总分也为 `null`；不得补 0，也不得对剩余维度重归一化。其他两态必须具有数值 score。

## 4. 固定评分规则

五维权重固定为 20% / 20% / 20% / 25% / 15%。复杂度固定为 1（简单）、2（中等）、3（复杂），只由目标、约束和交付范围判断；Token、时长、会话数、工具数、Skill 数和 Subagent 数不得影响复杂度或得分。

任务分是五维 `score * weight / 100` 之和。日分是有效任务分数按复杂度 1/2/3 加权的平均值。近 28 天分数是有效活跃日分数的等权平均；不足 3 个有效活跃日时等级必须为 `null`，展示“证据不足”。

统一使用 decimal `ROUND_HALF_UP`：只在 task、day、28-day 各级输出时保留 2 位；上一层使用已经规范化的下一层值计算；exact match 比较规范化后的两位小数。

基础分段为 L1 `0 <= score < 40`、L2 `40 <= score < 60`、L3 `60 <= score < 80`、L4 `80 <= score <= 100`，并应用 evidence gate：
...
- joint task name/project consistency：prediction 的 task name 以 case-sensitive exact match 匹配 gold `acceptedTaskNames` 之一，且 project 也 case-sensitive exact match 才记为正确；overall 必须 `>=80%`，每个 source tool 必须 `>=70%`。
